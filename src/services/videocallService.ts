...
export const videocallAPI = {
  ...
  createRoom: async (participantId: string): Promise<VideoCall> => {
    const response = await api.post('/videocall/create-room', { participantId });
    return response.data;
  },

  // Join a video call
  joinCall: async (callId: string): Promise<VideoCall> => {
    const response = await api.post(`/videocall/${callId}/join`);
    return response.data;
  },

  // End a video call
  endCall: async (callId: string): Promise<VideoCall> => {
    const response = await api.put(`/videocall/${callId}/end`);
    return response.data;
  },

  // Get call history
  getCallHistory: async (): Promise<VideoCall[]> => {
    const response = await api.get('/videocall/history');
    return response.data;
  },

  // Get active calls
  getActiveCalls: async (): Promise<VideoCall[]> => {
    const response = await api.get('/videocall/active');
    return response.data;
  },
};
